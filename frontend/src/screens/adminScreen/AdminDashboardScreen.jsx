...
import { UsersIcon, PackageIcon, ShoppingCartIcon, DollarSignIcon } from "lucide-react";
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';
    ...
    <div className="w-full p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className="bg-white shadow-md rounded-xl p-6 flex justify-between items-center transition hover:shadow-lg">
          <div>
            <p className="text-2xl font-bold">{totalOrders}</p>
            <p className="text-sm text-gray-500">{t('newOrders')}</p>
            <p className="text-sm text-green-500">{t('percentageChange')}</p>
          </div>
          <div className="bg-blue-100 text-blue-500 rounded-full p-3">
            <ShoppingCartIcon className="w-8 h-8" />
          </div>
        ...
        <div className="bg-white shadow-md rounded-xl p-6 flex justify-between items-center transition hover:shadow-lg">
          <div>
            ...
            <p className="text-sm text-green-500">{t('incomeChange')}</p>
          </div>
          <div className="bg-green-100 text-green-500 rounded-full p-3">
            <DollarSignIcon className="w-8 h-8" />
          </div>
        ...
        <div className="bg-white shadow-md rounded-xl p-6 flex justify-between items-center transition hover:shadow-lg">
          <div>
            ...
            <p className="text-sm text-green-500">{t('inventoryGrowth')}</p>
          </div>
          <div className="bg-yellow-100 text-yellow-500 rounded-full p-3">
            <PackageIcon className="w-8 h-8" />
          </div>
        ...
        <div className="bg-white shadow-md rounded-xl p-6 flex justify-between items-center transition hover:shadow-lg">
          <div>
            <p className="text-2xl font-bold">{totalUsers}</p>
            <p className="text-sm text-gray-500">{t('newUsers')}</p>
            <p className="text-sm text-red-500">{t('earningChange')}</p>
          </div>
          <div className="bg-purple-100 text-purple-500 rounded-full p-3">
            <UsersIcon className="w-8 h-8" />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Line Chart for Yearly Stats */}
        <div className="bg-white shadow-md rounded-xl p-6">
          <h2 className="text-lg font-bold mb-4">{t('yearlyStats')}</h2>
          <LineChart width={450} height={300} data={data}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="name" />
            <YAxis />
        ...
        {/* Bar Chart for Sales/Revenue */}
        <div className="bg-white shadow-md rounded-xl p-6">
          <h2 className="text-lg font-bold mb-4">{t('salesRevenue')}</h2>
          <BarChart width={450} height={300} data={data}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="name" />
            <YAxis />
