// import Address from "@/components/userScreenComponents/Address";
// import img from "../../assets/images/account.jpg";
// import { useDispatch, useSelector } from "react-redux";
// import UserCartItemsContent from "@/components/userScreenComponents/UserCartItemsContent";
...
function ShoppingCheckout() {
  const { cartItems } = useSelector((state) => state.shopCart);
  const { user } = useSelector((state) => state.auth);
  const { stripeSessionId } = useSelector((state) => state.shopOrder); // updated to stripeSessionId
  const [currentSelectedAddress, setCurrentSelectedAddress] = useState(null);
  const [isPaymentStart, setIsPaymemntStart] = useState(false);
  const dispatch = useDispatch();
  const { toast } = useToast();
  ...
  function handleInitiateStripePayment() {
    ...
    dispatch(createNewOrder(orderData)).then((data) => {
      ...
      if (data.payload.success) {
        setIsPaymemntStart(true);
        window.location.href = `https://checkout.stripe.com/pay/${data.payload.stripeSessionId}`; // Redirect to Stripe Checkout
      } else {
        setIsPaymemntStart(false);
      }
    });
